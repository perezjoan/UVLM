...
UVLM is distributed as a single Colab notebook (`UVLM.ipynb`) and requires only a Google account with access to a GPU runtime.
...
- Configure global generation parameters: temperature, top-p, max tokens (default: 50, range: 1–500), optional fixed random seed
...
- Write results to CSV: one row per image, one column per task, plus `{col}_raw` for raw responses
...
- Periodic checkpoint saves (every 5 images)
...
**Configuration**: Per-task checkbox + dropdown with two options: "2 runs" or "3 runs" (default: 2). Consensus is not available for `text` task type.
...
- Max tokens is overridden from default (50) to **1024** to accommodate the reasoning trace
...
- Reasoning trace stored in `{col}_reasoning` CSV column (truncated to 1000 chars)
...
### 3.6 CSV Schema and Resume Mode
...
header = (["image_name"]
          ...
          + reasoning_columns) # {col}_reasoning
```
...
- **100 images** of French street frontages
...
*Document version: v2.2.1 — February 2026*
